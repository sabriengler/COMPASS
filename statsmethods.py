...
def _ucl_count(mu_t, phi, alpha):
    ...
    if mu_t <= 1e-12:
        return 0.0
﻿
    # Poisson if no overdispersion
    if phi <= 1.0000001:
        return float(poisson.ppf(1 - alpha, mu_t))
﻿
    # NB plug-in approximation to match Var(Y) ≈ phi * mu
...
def farrington(
    ...
    baseline_period,
    alpha=0.01,
    period=52,             # 52 weekly, 365 daily
    b=5,                   # years/cycles back
    w=3,                   # +/- seasonal window
    no_periods=10,         # seasonal groups (Farrington Flexible style)
    reweight=True,
...
﻿
    ...
    out_of_control_index = None
﻿
    def seasonal_group(t_idx):
        pos = t_idx % period
        return int(np.floor(no_periods * pos / period))
﻿
...
﻿
        y_ref = data[idx]
        t_ref = np.asarray(idx, dtype=float)
﻿
        # seasonal factor groups
        g_ref = np.array([seasonal_group(j) for j in idx], dtype=int)
        season_ref = np.eye(no_periods, dtype=float)[g_ref][:, 1:]  # drop first level
﻿
        trend_ref = (t_ref - np.mean(t_ref)) / float(period)
﻿
        if include_trend:
