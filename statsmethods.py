...
def farrington(
    ...
    for t in range(baseline_period, n):
        ...
        season_ref = np.eye(no_periods, dtype=float)[g_ref][:, 1:]
...
def cusum(data, baseline_mean, sigma, sigma_multiplier, baseline_period=0, k_val=None, h_val=None):
    n = len(data)
    k = k_val * sigma if k_val is not None else 0.5 * sigma
    h = h_val * sigma if h_val is not None else sigma_multiplier * sigma
    c_plus = np.zeros(n)
    c_minus = np.zeros(n)
    out_of_control_index = None
