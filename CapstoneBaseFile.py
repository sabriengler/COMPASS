...
def apply_change_sim(data, change, change_day, params, original_behavior, baseline_mean, sigma, analysis_method, sigma_multiplier, baseline_period, lambda_val, max_days, alpha_val):
    max_days = 10000  # <--- ADD THIS LINE HERE
...
def plot_replicates_and_histogram(replications, run_lengths, change_day, analysis_method, sigma_multiplier, baseline_period, n_replications,
    ...
    # Mark the "detect within X days" threshold on the histogram
    if x is not None:
        hist_ax.axvline(x, color="black", linestyle="--", linewidth=2)
    ...
    # Add percentile and "within X days" stats
    stats += (f"\n\nRL Percentiles:\n"
              f"P10: {p10:.1f}\n"
              f"P25: {p25:.1f}\n"
              f"P50: {p50:.1f}\n"
    ...
    # UPDATED: Added 'fontsize=14' to make the text bigger
...
def optimize_lambda(data, method="ewma"):
    """
    Finds the optimal lambda to minimize variance (simplified optimization).
    """
    def objective(lam):
        if lam <= 0 or lam > 1: return 1e6
        # Simple proxy: minimize one-step-ahead prediction error on baseline
...
def run_simulation(behavior, params, n_baseline, change, change_day, analysis_method, n_replications, sigma_multiplier, max_days, lambda_val, late_threshold, alpha_val, custom_data=None):
    ...
    for _ in range(iterations):
        ...
        if custom_data is not None:
            ...
            # For custom data, we assume the data is static; we just scan it for signals
            # Reuse apply_change_sim? No, apply_change_sim generates NEW data.
            # We need a simple scanner or reuse the 'analysis' logic. 
            # Ideally, refactor 'apply_change_sim' to accept existing data, but for now:
            
            # Quick hack: We use the existing logic to SCAN only.
            # We call a helper or just the stat methods directly.
            # For simplicity, let's call the detection method directly here:
            out_idx = None
            if analysis_method == 'shewhart':
...
def index():
    ...
    if request.method == "POST":
        ...
        session['full_params'] = {
            ...
            "change": change, "change_day": change_day, "max_days": max_days, "late_threshold": late_threshold, "alpha_val": alpha_val
        }
        ...
        img, arl_value, chart_title, extra_stats = run_simulation(
            ...
            max_days, lambda_val, late_threshold,alpha_val, custom_data=custom_data
        )
        previous_results.append({"image": img, "title": chart_title, "arl": round(arl_value, 2), "rl_percentiles": extra_stats.get("rl_percentiles"),
    ...
    if request.method == "POST":
        ...
        if analysis_method in ["ewma", "mc-ewma"]:
            ...
            if lam_option == "manual":
                ...
                # Regenerate baseline just for optimization reference
        ...
        img, arl_value, chart_title, extra_stats = run_simulation(
            ...
            fp["max_days"], lambda_val, late_threshold, fp.get("alpha_val", 0.05), custom_data=None 
        )
        previous_results.append({"image": img, "title": chart_title, "arl": round(arl_value, 2), "rl_percentiles": extra_stats.get("rl_percentiles"),
                                 "detect_within_threshold_count": extra_stats.get("detections_within_x"),
                                 "detect_within_threshold_percent": extra_stats.get("percent_within_x"),
